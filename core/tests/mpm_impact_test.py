"""Impact test: drop each preset onto the floor and let it settle."""
import sys
import numpy as np
...
for i in range(24):
    for j in range(16):
        for k in range(16):
            positions[idx] = (0.8 + i * h * 0.5, 0.8 + j * h * 0.5, 0.4 + k * h * 0.5)
            idx += 1

for preset_name in ("Water", "Honey", "Jello", "Snow"):
    ...
    for frame in range(150):
        ...
        if pos[:, 2].min() < -1e-3 or pos[:, 2].max() > 2.4 + 1e-3:
            ok = False
            print(f"{preset_name:6s}: FAILED (z OOB at frame {frame})")
            break
    if ok:
